...
# 🧱 Page setup
st.set_page_config(page_title="Indiamart Scraper", page_icon="🏭", layout="centered")
...
# 🧾 Title
st.markdown('<div class="title">🏭 Indiamart B2B Scraper</div>', unsafe_allow_html=True)
...
if st.button("🚀 Scrape Product Data"):
    if not product_name.strip():
        ...
        filename = f"scraped_data/{product_name}.csv"
...
if product_name and os.path.exists(f"scraped_data/{product_name}.csv"):
    ...
    if st.button("🧠 Recommend the best product"):
        with st.spinner("The AI agent is thinking..."):
            product, reason = recommend_best_deal_with_ai(f"scraped_data/{product_name}.csv")
